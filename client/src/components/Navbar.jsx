...
export default function Navbar() {
  ...
  // --- THÊM: state điều khiển navbar mobile ---
  ...
  const goSearch = () => {
    const s = q.trim()
    if (!s) return
    if (suggestions.length === 1) navigate(suggestions[0].path)
    else navigate(`/search?q=${encodeURIComponent(s)}`)
    setOpen(false)
    setMobileOpen(false) // đóng menu mobile sau khi search
  }

  const onKeyDown = (e) => {
    if (e.key === 'Enter') { e.preventDefault(); goSearch() }
    else if (e.key === 'Escape') { setOpen(false); setProdOpen(false) }
  }
  ...
  return (
    <>
      {/* LOGO BAR (ẩn trên mobile bằng CSS để khỏi 2 logo) */}
      <div className={`logo-bar py-3 ${darkMode ? 'bg-dark' : 'bg-light'}`}>
        <div className="container d-flex justify-content-center">
          <Link to="/" className="text-decoration-none" onClick={closeMobile}>
      ...
      >
        <style>{`
          .search-pill {
            ...
            border-radius:999px; padding:.38rem .42rem .34rem .8rem; width:190px;
            border:1px solid transparent; transition: box-shadow .2s, border-color .2s, background .2s;
          }
          .search-input { border:none; outline:none; background:transparent; width:100%; font-size:.92rem; }
          .search-btn { width:34px; height:34px; border-radius:999px; border:0; outline:0; display:flex; align-items:center; justify-content:center; cursor:pointer; transition: transform .15s, box-shadow .2s; }
          .search-btn:active { transform: translateY(1px); }
          .search-suggest { position:absolute; right:0; top: calc(100% + .3rem); width:min(340px, 86vw); border-radius:12px; overflow:hidden; box-shadow:0 12px 28px rgba(0,0,0,.18); }
        ...
        <div className="container position-relative">
          {/* Logo nhỏ ở mobile – đã tăng size bằng CSS */}
          <Link className="navbar-brand d-lg-none" to="/" onClick={closeMobile}>
            <img src="/logo.png" alt="Logo" style={{ height: 56 }} />
          </Link>

          {/* TOGGLER – điều khiển bằng state mobileOpen */}
          <button
          ...
          {/* Collapse – dùng class show theo state, không phụ thuộc JS Bootstrap */}
          <div className={`collapse navbar-collapse justify-content-between ${mobileOpen ? 'show' : ''}`} id="mainNavbar">
            {/* Menu trái */}
            <ul className="navbar-nav me-auto mb-2 mb-lg-0">
              <li className="nav-item">
                <Link className="nav-link text-uppercase" to="/" onClick={closeMobile}>{t('home')}</Link>
              </li>

              <li className="nav-item">
                <Link className="nav-link text-uppercase" to="/gioi-thieu" onClick={closeMobile}>{t('about')}</Link>
              </li>
              ...
              >
                ...
                <ul className={`dropdown-menu ${prodOpen ? 'show' : ''}`}>
                  <li><Link className="dropdown-item" to="/thoi-trang-nam" onClick={() => { setProdOpen(false); closeMobile() }}>{t('men')}</Link></li>
                  <li><Link className="dropdown-item" to="/thoi-trang-nu" onClick={() => { setProdOpen(false); closeMobile() }}>{t('women')}</Link></li>
                  <li><Link className="dropdown-item" to="/thoi-trang-tre-em" onClick={() => { setProdOpen(false); closeMobile() }}>{t('kids')}</Link></li>
                </ul>
              </li>

              <li className="nav-item">
                <Link className="nav-link text-uppercase" to="/dich-vu-doi-tac" onClick={closeMobile}>
                  {t('partners')}
                </Link>
              </li>

              <li className="nav-item">
                <Link className="nav-link text-uppercase" to="/bai-viet" onClick={closeMobile}>{t('posts')}</Link>
              </li>
            </ul>
            ...
            <ul className="navbar-nav ms-auto mb-2 mb-lg-0 align-items-center">
              <li className="nav-item">
                <Link className="nav-link text-uppercase" to="/tuyen-dung" onClick={closeMobile}>{t('careers')}</Link>
              </li>

              <li className="nav-item">
                <Link className="nav-link text-uppercase contact-link" to="/contact" onClick={closeMobile}>{t('contact')}</Link>
              </li>
              ...
              <li className="nav-item dropdown">
                ...
                <ul className="dropdown-menu dropdown-menu-end" aria-labelledby="langDropdown">
                  <li><button className="dropdown-item" onClick={() => setLang('vi')}>VI — Tiếng Việt</button></li>
                  <li><button className="dropdown-item" onClick={() => setLang('en')}>EN — English</button></li>
                </ul>
              </li>
